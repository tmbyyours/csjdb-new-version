...
export default class ResLoader {
    ...
    public loadRes() {
        ...
        let finishCallback = (error: Error, resource: any) => {
            ...
            // Util.timeEnd("loadRes|" + resArgs.url);
        };
    ...
    public checkReleaseUse() {
        ...
        let item = this._getResItem(resArgs.url, resArgs.type);
        if (!item) {
            cc.log(`cant release,item is null ${resArgs.url} ${resArgs.type}`);
            return true;
        }
    ...
    async load_res_dir<T extends typeof cc.Asset>(path: string, type: T, item: any = null): Promise<InstanceType<T>[]> {
        return await new Promise(res => {
            cc.loader.loadResDir(path, type, (cur: number, tot: number) => {
                // cc.log("------------------------>>>>" + cur + "  /  " + tot + item)
                item != null && (item.progress != 'undefined' || item.progress != null) && (item.progress = (cur / tot));
            }, (err, resource) => {
    ...
    async load_res_by_type(cfg: {}, nd: any = null) {
        for (let item in cfg) {
            if (cfg[item].length != 0) {
                switch (item) {
                    case 'sounds':
                        for (let i = 0; i < cfg[item].length; i++) {
                            let clips = await resLoader.load_res_dir(cfg[item][i], cc.AudioClip, nd)
